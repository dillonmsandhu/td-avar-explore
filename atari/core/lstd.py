...
from core.buffer import LSTDBufferState
...
def solve_lstd_lambda_from_buffer(buffer: LSTDBufferState, Sigma_inv, config):
    ...
    def process_chunk(carry, chunk_data):
        A_acc, b_acc = carry
        phi, next_phi, traces, continue_mask, absorb_mask, mask = chunk_data
        mask = mask.squeeze()
        ...
        target_rho = get_scale_free_bonus(Sigma_inv, target_phi)
    ...
    (final_A, final_b), _ = jax.lax.scan(
        ...
        (
            ...
            chunked_next_phi, 
            chunked_traces, 
            chunked_continue_masks,
            chunked_absorb_masks, 
            chunked_mask
        )
    )
