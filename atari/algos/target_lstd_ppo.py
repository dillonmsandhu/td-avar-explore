...
def make_train(config):
    ...
    def train(rng):
        ...
        network, network_params = networks.initialize_actor_critic(rng, obs_shape, n_actions, 
                n_heads=2, cnn_torso=config.get('CNN_TORSO', 'CNN'), layer_norm= True, bias= True
        )

        train_state = networks.target_flax_train_state(
            config, network, network_params, network_params # target params
        )
        def get_lstd_feats(obs, target_params):
            return network.apply(target_params, obs, method=network.get_features)
        
        obsv, env_state = env.reset()
        initial_phi = get_lstd_feats(obsv, network_params)
        ...
        def _update_step(runner_state, unused):
            ...
            tau = config.get("TARGET_EMA_TAU", 0.01) # Default to 0.01 if not in config
            ...
            train_state = train_state.replace(target_params=new_target_params)
