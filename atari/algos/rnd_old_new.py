# RND but computes ri based on the gap in MSE before and after training.
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_scan_state, unused):
                ...
                predictor_mse = 0.5 * ((rnd_target_feats - rho_feats)**2).sum(-1)
                ...
                transition = Transition(
                    done, action, ve, next_ve, vi, next_vi, reward, 0.0, predictor_mse, log_prob, last_obs, next_rnd_obs, obsv, rnd_target_feats, info
                )
            ...
            (train_state, env_state, last_obs, obs_rms, rng) = updated_env_step_state
            
            # Train the predictor
            def update_epoch_rnd(update_state, _):
                rnd_state, traj_batch, rng = update_state
                rng, _rng = jax.random.split(rng) 
                minibatches = helpers.shuffle_and_batch(_rng, traj_batch, config["NUM_MINIBATCHES"])
                rng, mask_rng = jax.random.split(rng)

                def mb_rnd(carry, minibatch):
                    rnd_state, mask_rng = carry
                    # --- UPDATE RND ---
                    def rnd_loss_fn(rnd_params):
                        pred = rnd_net.apply(rnd_params, minibatch.next_rnd_obs)
                        # FIX 2: Use minibatch, not traj_batch
                        diff = pred - minibatch.rnd_target_feats 
                        losses = jnp.mean(jnp.square(diff), axis=-1)
                        loss = (losses * mask).sum() / (mask.sum() + 1e-8)
                        return loss
                    
                    rnd_grad_fn = jax.value_and_grad(rnd_loss_fn)
                    mask = jax.random.bernoulli(mask_rng, p=config.get('RND_TRAIN_FRAC',0.25), shape=(minibatch.obs.shape[0],))
                    ...
                    return (rnd_state, mask_rng), rnd_loss # return mask_rng in carry
                
                initial_mb_carry = (rnd_state, mask_rng)
                final_mb_carry, rnd_loss = jax.lax.scan(mb_rnd, initial_mb_carry, minibatches)
                rnd_state, _ = final_mb_carry
                
                return (rnd_state, traj_batch, rng), rnd_loss
            
            # Update RND State
            initial_update_state = (rnd_state, traj_batch, rng)
            update_state, rnd_loss = jax.lax.scan(update_epoch_rnd, initial_update_state, None, config["NUM_EPOCHS"])
            rnd_state, _, rng = update_state
            
            # Compute ri:
            # Compute the gap in prediction error:
            mse1 = traj_batch.predictor_mse
            rho_feats = rnd_net.apply(rnd_state.params, traj_batch.next_rnd_obs)
            mse2 = 0.5 * ((traj_batch.rnd_target_feats - rho_feats)**2).sum(-1)
            # MSE 2 should generally be smaller since this data point was in the training set.
            gap = jnp.maximum(0, mse1 - mse2)
            ri_raw = gap
            
            # Compute intrinsic return, normalize, and update traj_batch.ri
            def _normalize_intrinsic_reward(carry, ri_raw_t):
                irets_carry, iret_rms_carry = carry
                # Forward exponentially discounted return
                irets_carry = ri_raw_t + config['GAMMA_i'] * irets_carry
                # Update the running statistics
                iret_rms_carry = helpers.update_rms(iret_rms_carry, irets_carry)
                # Normalize the raw reward (with epsilon for numerical stability)
                ri_t = ri_raw_t / jnp.sqrt(iret_rms_carry["var"] + 1e-8)
                return (irets_carry, iret_rms_carry), ri_t

            # Execute the scan over the NUM_STEPS dimension (axis 0)
            norm_carry = (irets, iret_rms)
            (irets, iret_rms), ri_normalized = jax.lax.scan(
                _normalize_intrinsic_reward, 
                norm_carry, 
                ri_raw
            )

            # 3. Inject the normalized rewards back into the trajectory batch
            # (Assuming your Transition NamedTuple has an 'intrinsic_reward' field)
            traj_batch = traj_batch._replace(intrinsic_reward=ri_normalized)
