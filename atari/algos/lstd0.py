        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_scan_state, unused):
                ...
                transition = Transition(
                    done, is_goal, action, value, next_val, dummy, dummy, reward, dummy, log_prob, last_obs, obsv, info
                )
            ...
            lstd_state = solve_lstd_0_from_buffer(buffer_state, Sigma_inv, config)
            ...
            gaes, targets = helpers.calculate_gae(
                ...
                config["GAMMA"], 
                config["GAE_LAMBDA"], 
                is_continuing, # never cut
                is_absorbing, # cut on deaths, overwrite goals (S_T -> S_0) to absorbing (S_T -> S_T -> S_T ...)
                γi=config["GAMMA_i"], 
