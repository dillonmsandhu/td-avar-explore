# run_exact.py
# Runs only those that are solved exactly
from core.configs import EXACT_REGISTRY
import subprocess
import datetime
import argparse
import datetime
...
def run_experiment():
    ...
    for config_name, details in EXACT_REGISTRY.items():
        envs = details["envs"]
...
def summarize_batch(batch_id, results_root):
    ...
    for env_name in envs_found:
        try:
            ...
            group = next(
                (g for g, d in EXACT_REGISTRY.items() if env_name in d["envs"]),
                "other",
            )
