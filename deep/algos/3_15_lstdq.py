...
def make_train(config):
    ...
    def lstd_q_batch_update(lstd_state: Dict, transitions, phi_sa, expected_next_phi, lambda_s, rho_unscaled):
        """
        LSTD-Q update evaluating the Uniform Random Policy.
        No trace function is used; it purely relies on phi_sa -> expected_next_phi.
        """
        ...
        # Optional Importance Sampling matching original behavior (though less strictly necessary for off-policy LSTD-Q)
        is_weights = rho_unscaled ** 2 
        is_weights = is_weights / (is_weights.mean() + 1e-8)

        # 1. Standard LSTD-Q A update
        ...
        # 2. Optimism: Apply IS Weighting to the Prior Injection
        ...
        # Non-optimistic EMA for storage
        A_i = helpers.EMA(alpha_fn_lstd(t), lstd_state["A"], A_batch)

        # 3. Feature-reward vector (b) based purely on phi_sa
        b_i_sample = phi_sa * rho[..., None]
        b_i_sample = b_i_sample * is_weights[..., None] # Empirical b IS weighted

        b_i_sample_opt = (1-lambda_s)[..., None] * b_i_sample  + (lambda_s * is_weights)[..., None] * lstd_state['V_max'] * phi_sa
    ...
    def train(rng):
        ...
        rnd_net, rnd_params = networks.initialize_rnd_network(rnd_rng, obs_shape, config['RND_NETWORK_TYPE'],config['NORMALIZE_FEATURES'], config['BIAS'], k, state_action_features=True, n_actions=action_dim)
