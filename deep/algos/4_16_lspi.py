...
from core.buffer import LSPIFeatureBufferManager, LSPIBufferState
from core.lstd import solve_lspi_buffer
# jax.config.update("jax_enable_x64", True)

SAVE_DIR = "4_16_lspi" # performs the update to the covariance matrix FIRST.
...
def make_train(config):
    ...
    buffer_manager = LSPIFeatureBufferManager(config, k_lstd, BUFFER_CAPACITY, EXTENDED_CAPACITY, config['CHUNK_SIZE'], dim_kA, n_actions) # stateless buffer manager.
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            buffer_batch = LSPIBufferState(phi_sa, next_phi_s, terminals, absorb_masks, size=jnp.array(batch_size))
