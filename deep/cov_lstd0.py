...
def make_train(config):
    ...
    def interpolate_lstd_val(lstd_state, ri, S, phi_fn=None, obs=None, phi=None):
        ...
        N_eff = jnp.einsum('...i,ij,...j->...', features, S_inv, features)
        
        # Interpolation Method
        if config.get('VMAX_INTERPOLATE_LINEAR', False):
            N0 = config.get('EFFECTIVE_VISITS_TO_REMAIN_OPT', 10)
            eps = N0 / N_eff # uncertainty
        else:
            eps = 1.0 / jnp.sqrt(N_eff) # uncertainty
        
        # Interpolation
        weight_v_max = jnp.clip(eps, 0.0, 1.0)
    ...
    def lstd_batch_update(lstd_state: Dict, transitions, features, next_features):        
        ...
        b_int_sample = features * rho[..., None]
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            vi = interpolate_lstd_val(lstd_state, int_rew_from_features(phi),sigma_state['S_long'],phi=phi)
            vi_baseline = phi @ lstd_state["w"]
            traj_batch = traj_batch._replace(i_value_fast=vi, i_value_slow=vi_baseline)

            # Advantage
            _, last_val = network.apply(train_state.params, last_obs)
            last_i_val_fast = interpolate_lstd_val(lstd_state, rho, sigma_state['S_long'], phi=next_phi[-1])
