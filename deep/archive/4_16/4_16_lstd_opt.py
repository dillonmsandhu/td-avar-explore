# uses LSPI-Rmax for optimism injection. 
# if a state's counts are low, increase its value
from core.imports import *
import core.helpers as helpers
import core.networks as networks
# jax.config.update("jax_enable_x64", True)

SAVE_DIR = "4_16_lstd_opt" # performs the update to the covariance matrix FIRST.
...
def make_train(config):
    ...
    def solve_lstd_buffer(buffer_state: LSTDBufferState, Sigma_inv, lstd_state, config):
        ...
        def process_chunk(carry, chunk_data):
            ...
            # optimism:
            c_rho = get_scale_free_bonus(Sigma_inv, phi)
            n_eff = 1.0 / (c_rho**2 + 1e-8)          
            prior_weight = config['LSTD_PRIOR_SAMPLES'] / (n_eff + 1e-8)  

            prior_traces = phi * prior_weight[:, None] * mask
            
            # Add the weighted outer product to A 
            A_prior = jnp.einsum("ni, nj -> ij", prior_traces, phi)
            
            # Target V_max_raw for the b vector
            b_prior = jnp.einsum("ni, n -> i", prior_traces, jnp.ones_like(c_rho) * V_max)
            ...
            A_batch = A_batch + A_prior + A_abs
            b_batch = b_batch + b_prior + b_abs

            return (A_batch, b_batch), None
    ...
    def evict_buffer(buffer_state: LSTDBufferState, config, rng):
        ...
        def cut_step(carry, step_idx):
            ...
            reg = jnp.eye(k_lstd) * config.get("LSTD_L2_REG", 1e-3) * size
            A_inv_curr = jnp.linalg.solve(A_curr + reg, jnp.eye(k_val)) 
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            rho = get_scale_free_bonus(Sigma_inv, next_phi)
