...
class LSPIBufferState(NamedTuple):
    traces: jnp.ndarray        # shape: (..., dim_kA)
...
def make_train(config):
    ...
    def update_phi_precision(lstd_state, phi_sa, next_phi_s, absorb_masks):
        phi_diag_precision = lstd_state['phi_diag_counts']
        
        # Calculate precision for this batch's state-action pairs
        batch_phi_prec = jnp.sum(phi_sa**2, axis=(0, 1)) 
        
        # Calculate precision for the absorbing state assuming all actions are taken equally
        Pi_all_actions = jnp.ones((*next_phi_s.shape[:-1], n_actions))
        all_action_abs_features = expected_next_sa_features(next_phi_s, Pi_all_actions) * absorb_masks[..., None]
        abs_phi_prec = jnp.sum(all_action_abs_features**2, axis=(0, 1)) 
    ...
    def update_buffer(buffer_state: LSPIBufferState, traces, features_sa, next_features_s, terminals, absorb_masks):
        ...
        traces = traces.reshape(-1, dim_kA).astype(jnp.float32)
    ...
    def solve_lspi_buffer(buffer_state: LSPIBufferState, Sigma_inv, lstd_state, config):
        ...
        def lspi_step(w_current, _):
            ...
            w_new = jnp.linalg.pinv(A_view, rtol=1e-8) @ b_view
    ...
    def train(rng):
        initial_lstd_state = {
                    ...
                    "V_max": V_max,
                    "phi_diag_counts": jnp.zeros(dim_kA),
                }
            
        initial_buffer_state = LSPIBufferState(
            traces=jnp.zeros((PADDED_CAPACITY, dim_kA)),
        ...
        def _update_step(runner_state, unused):
            ...
            lstd_state = update_phi_precision(lstd_state, phi_sa, next_phi_s, absorb_mask_batch)
            ...
            batch_next_rho = get_scale_free_bonus(Sigma_inv, next_phi_s)
            ...
            metric.update(
                {
                    ...
                    "lambda_kA": lstd_state['phi_diag_counts'],
                    "beta": lstd_state["Beta"],
                    "rho_scale": rho_scale,
                }
            )
