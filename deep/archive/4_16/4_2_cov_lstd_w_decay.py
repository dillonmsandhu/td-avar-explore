...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            # Two norm:
            # w_e_norm = jnp.linalg.norm(lstd_state["w_e"])
            # Max norm:
            w_e_norm = jnp.max(jnp.abs(lstd_state["w_e"]))
            # w_e_norm_2 = jnp.linalg.norm(lstd_state["w_e"])
            # w_e_norm = jnp.minimum(w_e_norm_inf, w_e_norm_2)
            scaling_factor = jnp.minimum(lstd_state['V_max'], w_e_norm)
            # dynamic_beta = config["BONUS_SCALE"] + config["BONUS_SCALE"] * w_e_norm
