...
def make_train(config):
    ...
    ema_alpha_fn = lambda idx: jnp.maximum(1/(idx+1), config.get('MIN_LSTD_LR', 1/10))
    ...
    def solve_lstd_buffer(buffer_state: LSTDBufferState, Sigma_inv, lstd_state, alpha, config):
        ...
        def process_chunk(carry, chunk_data):
            ...
            N_batch = jnp.sum(mask)
            return (A_acc + A_batch + A_abs, b_acc + b_batch + b_abs, N_acc + N_batch), None

        init_A = jnp.zeros((k_lstd, k_lstd))
        init_b = jnp.zeros(k_lstd)
        
        (final_A, final_b, N), _ = jax.lax.scan(
            process_chunk, 
            (init_A, init_b, 0.0), 
        ...
        Lambda_mat = jnp.diag(lambda_k) / N
        prior_b = lambda_k * lstd_state["V_max"] / N
        
        # 5. Regularize and Solve ONLY ONCE at the end
        reg = jnp.eye(k_lstd) * config["A_REGULARIZATION_PER_STEP"] / N

        A_ema = (1-alpha) * lstd_state["A_ema"] + alpha * (final_A / N)

        A_view = A_ema + Lambda_mat + reg
        b_view = (final_b / N + prior_b)
    ...
    def evict_buffer(buffer_state: LSTDBufferState, config, rng):
        """Pure FIFO eviction using fast memory slicing."""
        size = buffer_state.size
        buffer_is_full = size > BUFFER_CAPACITY
        batch_size = config["NUM_STEPS"] * config["NUM_ENVS"]
        
        def shift_buffer(state):
            # The buffer has overshot by exactly 'batch_size'. 
            # The oldest items are at indices 0 to batch_size - 1.
            # We slice from 'batch_size' to the end, effectively dropping the oldest batch.
            
            new_traces = jnp.zeros_like(state.traces).at[:BUFFER_CAPACITY].set(
                state.traces[batch_size:BUFFER_CAPACITY + batch_size]
            )
            new_features = jnp.zeros_like(state.features).at[:BUFFER_CAPACITY].set(
                state.features[batch_size:BUFFER_CAPACITY + batch_size]
            )
            new_next_features = jnp.zeros_like(state.next_features).at[:BUFFER_CAPACITY].set(
                state.next_features[batch_size:BUFFER_CAPACITY + batch_size]
            )
            new_terminals = jnp.zeros_like(state.terminals).at[:BUFFER_CAPACITY].set(
                state.terminals[batch_size:BUFFER_CAPACITY + batch_size]
            )
            new_absorb_masks = jnp.zeros_like(state.absorb_masks).at[:BUFFER_CAPACITY].set(
                state.absorb_masks[batch_size:BUFFER_CAPACITY + batch_size]
            )
            
            return LSTDBufferState(
                traces=new_traces, features=new_features, next_features=new_next_features,
                terminals=new_terminals, absorb_masks=new_absorb_masks,
                size=jnp.array(BUFFER_CAPACITY, dtype=jnp.int32)
            )
            
        def keep_buffer(state):
            # If we haven't hit capacity yet, do nothing.
            return state
            
        # jax.lax.cond ensures we only do the memory shifting if the buffer actually overflowed
        return jax.lax.cond(buffer_is_full, shift_buffer, keep_buffer, buffer_state)
    ...
    def train(rng):
        initial_lstd_state = {
                    "A_ema":  jnp.eye(k_lstd, dtype=jnp.float64),
                    "w": jnp.zeros(k_lstd), 
        ...
        def _update_step(runner_state, unused):
            ...
            alpha_A = ema_alpha_fn(idx)
            lstd_state = solve_lstd_buffer(buffer_state, Sigma_inv, lstd_state, alpha_A, config)
