...
SAVE_DIR = '4_8_lstd_vi_feats_buffer'
...
def make_train(config):
    ...
    def evict_buffer(buffer_state: LSTDBufferState, A_inv, vi_params, feature_fn, config):
        size = buffer_state.size
        
        # --- Option: IV Leverage Scores or FIFO ---
        if config.get("PRIORITIZED_EVICTION", False):
            ...
            chunked_next_obs = buffer_state.next_obs.reshape(NUM_CHUNKS, CHUNK_SIZE, *obs_shape)
            chunked_terminals = buffer_state.terminals.reshape(NUM_CHUNKS, CHUNK_SIZE, 1)
            chunked_mask = valid_mask.reshape(NUM_CHUNKS, CHUNK_SIZE)
            
            gamma_i = config["GAMMA_i"]

            def score_chunk(carry, chunk_data):
                c_obs, c_next_obs, c_term, c_mask = chunk_data
                
                c_phi = feature_fn(vi_params, c_obs)
                c_next_phi = feature_fn(vi_params, c_next_obs)
                
                Z = c_phi
                X = c_phi - gamma_i * (1 - c_term) * c_next_phi
                ...
                scores = jnp.where(c_mask, scores, -jnp.inf)
                return None, scores

            _, chunked_scores = jax.lax.scan(score_chunk, None, (chunked_obs, chunked_next_obs, chunked_terminals, chunked_mask))
            scores = chunked_scores.reshape(PADDED_CAPACITY)
        ...
        # Safely compact elements from keep_indices down into the front of the array
    ...
    def train(rng):
        ...
        def get_vi_features(params, obs_data):
            ...
            if config.get("L2_NORMALIZE_VI_FEATURES", True):
        ...
        def _update_step(runner_state, unused):
            ...
            # 4. Score and Evict
            # JAX's compiler will optimize out A_inv entirely if PRIORITIZED_EVICTION is False
            A_inv = jnp.linalg.inv(A_view)
            buffer_state = evict_buffer(buffer_state, A_inv, vi_params, get_vi_features, config)
