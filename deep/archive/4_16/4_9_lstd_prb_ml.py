...
SAVE_DIR = "4_9_lstd_prb_ml"

def generate_maclaurin_params(rng, in_dim, out_dim):
    """
    Generates two independent Stacked Orthogonal Matrices for a Degree-2 Polynomial Sketch.
    """
    def get_orthogonal_matrix(key):
        n_blocks = (out_dim + in_dim - 1) // in_dim
        blocks = []
        for i in range(n_blocks):
            z = jax.random.normal(jax.random.fold_in(key, i), (in_dim, in_dim))
            q, r = jnp.linalg.qr(z)
            d = jnp.diag(r)
    ...
    W1 = get_orthogonal_matrix(key1)
    W2 = get_orthogonal_matrix(key2)
...
def make_train(config):
    ...
    def train(rng):
        ...
        # Initialize the static Maclaurin sketch state
        mac_rng, rng = jax.random.split(rng)
        W1_mac, W2_mac = generate_maclaurin_params(mac_rng, in_dim=k_base, out_dim=k_sketch)
        ...
        def get_phi_lstd(phi_base):
            ...
            # 2. Degree-2 Polynomial Sketch
            proj1 = phi_norm @ W1_mac
            proj2 = phi_norm @ W2_mac
            phi_sketch = (proj1 * proj2) / jnp.sqrt(k_sketch)
            
            # 3. Hierarchical Concatenation
            bias = jnp.ones(phi_norm.shape[:-1] + (1,))
            return jnp.concatenate([bias, phi_norm, phi_sketch], axis=-1)
