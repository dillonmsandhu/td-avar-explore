# Inverts the MEAN covariance matrix ....
...
def make_train(config):
    ...
    def train(rng):
        ...
        initial_sigma_state = {'S': jnp.eye(k,dtype=jnp.float64), 'N': 1}
        ...
        def _update_step(runner_state, unused):
            ...
            # 2. Invert
            cho_S = jax.scipy.linalg.cho_factor(sigma_state["S"] + eps * jnp.eye(k)) 
            S_inv = jax.scipy.linalg.cho_solve(cho_S, jnp.eye(k)) 
        ...
        rng, _rng = jax.random.split(rng)
        init_runner_state = (train_state, initial_sigma_state, rnd_state, env_state, obsv, config['BONUS_SCALE'], _rng, 1)
