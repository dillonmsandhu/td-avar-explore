...
SAVE_DIR = "4_8_lstd_prb_maclaurin"
...
def make_train(config):
    ...
    k_sketch = config.get("SKETCH_FEATURES", 512) # Up to 1024 if memory permits
    ...
    def train(rng):
        ...
        # --- 2. RADEMACHER SKETCH INITIALIZATION ---
        w1_rng, w2_rng, rng = jax.random.split(rng, 3)
        W1_rad = (jax.random.bernoulli(w1_rng, 0.5, (k_base, k_sketch)).astype(jnp.float32) * 2.0 - 1.0)
        W2_rad = (jax.random.bernoulli(w2_rng, 0.5, (k_base, k_sketch)).astype(jnp.float32) * 2.0 - 1.0)
        
        def apply_sketch(phi_128):
            """Unbiased degree-2 polynomial expansion using Random Maclaurin."""
            proj1 = jnp.dot(phi_128, W1_rad)
            proj2 = jnp.dot(phi_128, W2_rad)
        ...
        def solve_lstd_buffer(buffer_state: LSTDBufferState, Sigma_inv, lstd_state, config):
            ...
            def process_chunk(carry, chunk_data):
                ...
                aug_traces = c_traces * c_mask  # traces are already augmented/sketched
        ...
        def _update_step(runner_state, unused):
            ...
            if evaluator is not None:
                ...
                def get_vi(obs):
                    # Manual reproduction of the exact analytical expansion for evaluation
