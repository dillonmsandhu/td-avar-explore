...
SAVE_DIR = "4_13_lstd_learned_feats"
...
class FeatureNet(nn.Module):
    ...
    def setup(self):
        ...
        # 2. LSTD Feature Component (Unbiased internals, explicit appended bias)
        learned_dim = self.k_lstd - 1 if self.lstd_bias else self.k_lstd
        self.lstd_feature_layer = nn.Dense(learned_dim, use_bias=False, kernel_init=orthogonal(jnp.sqrt(2)))
        ...
        # 4. Residual Delta Head (For expected next state)
    ...
    def _get_normalized_lstd_features(self, x):
        ...
        phi_raw = self.lstd_feature_layer(z_int) 
...
def make_train(config):
    ...
    def train(rng):
        ...
        feat_tx = optax.chain(
            ...
            optax.adam(learning_rate=config["LR"]/5, b1=0.0, eps=1e-5)
        )
