...
def add_values_to_metric(config, metric, int_rew_from_state, evaluator, old_beta, network, train_state, traj_batch, get_vi = None, get_ve = None, rho_scale=None, compute_true_vals = True):
    ...
    if "DeepSea" in env_name:
        metric['visitation_count'] = obs.sum(axis=(0, 1))    
...
def initialize_evaluator(config):
    ...
    evaluator = None
    if config['ENV_NAME'] == 'Maze':
        evaluator = SparseMazeExactValue(episodic=episodic, absorbing = absorbing, gamma=config['GAMMA_i'], )
...
def update_cov_mean(traj_batch, sigma_state, get_features_fn):
    "Updates traj_batch and sigma_state based on feature visitations."
    ...
    def cov_update_masked(
        ...
        # Zeros out the outer products corresponding to invalid/padding states
        S_masked = S_update * mask[..., None, None]
        N_old = sigma_state.get('N', 0) 
        N_new = N_old + jnp.sum(mask)
        # 3. PURE SUMMATION (No division!)
        batch_axes = tuple(range(mask.ndim))
        S_batch_sum = jnp.sum(S_masked, axis=batch_axes)
        
        S_new = (N_old / N_new) * S + (1 / N_new) * S_batch_sum
        S_new = 0.5 * (S_new + S_new.T)
...
def make_sine_schedule(total_updates: int, max_beta: float):
    """
    Smooth cosine/sine schedule: 0 -> max_beta -> 0.
    Follows a half-sine wave, peaking halfway through training.
    """
    def schedule(step):
        progress = step / total_updates
        # sin(0) = 0, sin(pi/2) = 1, sin(pi) = 0
        multiplier = jnp.sin(jnp.pi * progress)
        return max_beta * multiplier
    return schedule