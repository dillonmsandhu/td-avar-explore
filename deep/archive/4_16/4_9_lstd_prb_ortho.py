...
SAVE_DIR = "4_9_lstd_prb_orf"

def generate_orf_params(rng, in_dim, out_dim, sigma=1.0):
    """
    Generates parameters for Gaussian Orthogonal Random Features.
    """
    key_q, key_s, key_p = jax.random.split(rng, 3)
    
    # 1. Generate Orthogonal Matrix Q (Haar Measure)
    # We use the same stacking logic as before
    n_blocks = (out_dim + in_dim - 1) // in_dim
    blocks = []
    for i in range(n_blocks):
        z = jax.random.normal(jax.random.fold_in(key_q, i), (in_dim, in_dim))
        q, r = jnp.linalg.qr(z)
        d = jnp.diag(r)
        q = q * (d / jnp.abs(d)) # Ensure Haar distribution
        blocks.append(q)
    Q = jnp.concatenate(blocks, axis=1)[:, :out_dim]
    
    # 2. Generate Chi-scaling (S) 
    # This makes the rows have the same radial length as a d-dim Gaussian
    # We sample squared lengths from a Chi-squared distribution
    s_sq = jax.random.chisquare(key_s, df=in_dim, shape=(out_dim,))
    S = jnp.diag(jnp.sqrt(s_sq))
    
    # Combine to get W = Q @ S
    # The sigma parameter scales the kernel bandwidth
    W_orf = (Q @ S) / sigma
    
    # 3. Random Phase
    phase = jax.random.uniform(key_p, (out_dim,), minval=0, maxval=2 * jnp.pi)
...
def make_train(config):
    ...
    def train(rng):
        ...
        # initialize the static ORF state
        orf_rng, rng = jax.random.split(rng)
        W_orf, orf_phase = generate_orf_params(orf_rng, in_dim=k_base, out_dim=k_sketch, sigma=jnp.sqrt(k_lstd) / 2.0)
        
        def get_phi_lstd(phi_base):
            phi_relu = nn.relu(phi_base)
            norm = jnp.linalg.norm(phi_relu, axis=-1, keepdims=True)
            phi_norm = phi_relu / jnp.maximum(norm, 1e-8)
            proj = phi_norm @ W_orf
            phi_gaussian = jnp.sqrt(2.0 / k_sketch) * jnp.cos(proj + orf_phase)
            bias = jnp.ones(phi_norm.shape[:-1] + (1,))
            return jnp.concatenate([bias, phi_norm, phi_gaussian], axis=-1)
