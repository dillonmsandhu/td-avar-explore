...
def make_train(config):
    ...
    def solve_lspi_buffer(buffer_state: LSPIBufferState, Sigma_inv, lstd_state, config):
        ...
        def lspi_step(w_current, _):
            def process_chunk(carry, chunk_data):
                ...
                return (A_acc + A_batch, b_acc + b_batch, count + jnp.sum(c_mask)), None
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            # 1. Precision from the actual state-action pairs taken in the environment
            batch_sa_precision = (phi_sa**2).sum(axis=(0, 1))
            # 2. Precision for the absorbing state (The Black Hole)
            # Array of 1.0s to simulate taking EVERY action fully. 
            Pi_all_actions = jnp.ones((*traj_batch.done.shape, n_actions))
            all_action_abs_features = expected_next_sa_features(next_phi_s, Pi_all_actions) * absorb_masks[..., None]
            absorbing_sa_precision = (all_action_abs_features**2).sum(axis=(0, 1))
            
            new_phi_diag_counts = lstd_state["phi_diag_counts"] + batch_sa_precision + absorbing_sa_precision
            lstd_state = {**lstd_state, "phi_diag_counts": new_phi_diag_counts}
