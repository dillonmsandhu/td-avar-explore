...
def make_train(config):
    ...
    def solve_lstd_buffer(buffer_state: LSTDBufferState, S_inv, lstd_state, config):
    ...
    def evict_buffer(buffer_state: LSTDBufferState, config, rng):
        ...
        def cut_step(carry, step_idx):
            ...
            # Rebuild A matrix using ONLY the currently surviving points
            ...
            A_inv_curr = jnp.linalg.pinv(A_curr, rtol=1e-8) # rtol is lowest condition number dropped.
            ...
            # Extract Sherman-Morrison scalars
            ...
            # Compute the exact variance penalty per transition
            ...
            # Force already-dropped items to -inf so they aren't dropped again
            drop_logits = jnp.where(mask_curr, drop_logits, -jnp.inf)
            
            # Inject Gumbel noise for sampling without replacement
            ...
            # JAX SAFETY: Use the static integer!
            _, drop_indices = jax.lax.top_k(noisy_logits, static_drops_per_cut)
            
            # Update the mask (set dropped indices to False)
            mask_next_candidate = mask_curr.at[drop_indices].set(False)
            
            # ONLY apply these drops if the buffer is actually over capacity
    ...
    def train(rng):
        ...
        initial_sigma_state = {"S": jnp.eye(k_lstd, dtype=jnp.float64), "N": 1} # global accumulation
        ...
        def _update_step(runner_state, unused):
            ...
            # --- ρ from S: MEAN COVARIANCE --
            current_N = sigma_state["N"]

            # 1. Scale epsilon using the exact N that generated S
            eps = jnp.maximum(config['GRAM_REG'] / current_N, 1e-9)

            # 2. Invert
            cho_S = jax.scipy.linalg.cho_factor(sigma_state["S"] + eps * jnp.eye(k_lstd)) 
            S_inv = jax.scipy.linalg.cho_solve(cho_S, jnp.eye(k_lstd)) 

            # 3. Scale rho using the exact N
            rho_scale = config['BONUS_SCALE'] / jnp.sqrt(current_N)
            
            # --- 2. SOLVE LSTD OVER EXTENDED BUFFER ---
            lstd_state = update_phi_precision(lstd_state, phi, next_phi, traj_batch.done)
            lstd_state = solve_lstd_buffer(buffer_state, S_inv, lstd_state, config)
            ...
            batch_curr_rho = get_scale_free_bonus(S_inv, phi) * rho_scale
            batch_next_rho = get_scale_free_bonus(S_inv, next_phi) * rho_scale
            ...
            traj_batch = traj_batch._replace(i_value=v_i * rho_scale, intrinsic_reward=batch_next_rho, next_i_val=next_v_i * rho_scale)
            ...
            sigma_state = helpers.update_cov_mean(traj_batch, sigma_state, batch_get_features)
            
            eigvals_S = jnp.linalg.eigvalsh(sigma_state["S"])
            cond_S = eigvals_S[-1] / jnp.maximum(eigvals_S[0], 1e-12)
            cond_regularized = (eigvals_S[-1] + eps) / (eigvals_S[0] + eps)
            # --------- Metrics ---------
            metric = {
                k: v.mean() 
            ...
            metric.update(
                {
                    ...
                    "Condition_Number_S_reg": cond_regularized,
                }
            )
