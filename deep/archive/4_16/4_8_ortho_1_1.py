...
def make_train(config):
    ...
    def evict_buffer(buffer_state: LSTDBufferState, A_view, Sigma_inv, config):
        size = buffer_state.size
        
        if config.get("PRIORITIZED_EVICTION", True):
            # 1. Reconstruct Augmented Features and Temporal Differences
            ...
            terminals_all = buffer_state.terminals
            gamma_i = config["GAMMA_i"]
            
            X_all = aug_phi - gamma_i * (1 - terminals_all) * aug_next_phi
            
            # 2. Sequential Sherman-Morrison One-by-One Downdates
            initial_mask = jnp.arange(PADDED_CAPACITY) < size
            initial_A_inv = jnp.linalg.pinv(A_view, rcond=1e-5) # Use pinv for initial shock absorption
        ...
        # 3. Compaction (Mapping specifically to the Ortho Buffer State arrays)
        ...
        return LSTDBufferState(
            traces=new_traces, features=new_features, next_features=new_next_features,
            terminals=new_terminals, absorb_masks=new_absorb_masks, size=new_size
        )
