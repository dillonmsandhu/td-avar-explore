...
def make_train(config):
    ...
    def train(rng):
        ...
        initial_sigma_state = {"S": jnp.zeros((k_lstd, k_lstd)).astype(jnp.float64), "N": 1}
        ...
        def _update_step(runner_state, unused):
            ...
            traj_batch = traj_batch._replace(intrinsic_reward=batch_next_rho)

            # Get Oracle (LSTD) Values
            lstd_v_i = phi @ lstd_state["w"]
            lstd_next_v_i = next_phi @ lstd_state["w"]
            lstd_v_i, lstd_next_v_i = jax.tree.map(lambda x: jnp.clip(x, 0, 1.0/(1-config['GAMMA_i'])) , (lstd_v_i, lstd_next_v_i) )
            
            # LSTD preds produce the TD(lambda) Distillation Targets
            ...
            distilled_i_target = lstd_targets[1] # TD Target

            # Network GAEs to produce On-Policy Actor Advantages
            ...
            if evaluator is None: 
                ...
                def get_vi(obs):
                    _, _, i_val = network.apply(train_state.params, obs) * rho_scale
                    return i_val
