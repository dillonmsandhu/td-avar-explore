...
SAVE_DIR = "4_6_lstd_cumulant_feats"
...
def make_train(config):
    ...
    k_val = k_rnd  # Cumulant network will match the RND dimension
    ...
    # --- HYBRID SOLVER: RND for Reward, CUMULANT Features for Value ---
    def solve_lstd_buffer(buffer_state: LSTDBufferState, rnd_params, rnd_net, cumulant_params, cumulant_net, Sigma_inv, k_val, config):
        CHUNK_SIZE = 10_000  
        ...
        def process_lstd_chunk(carry, chunk_data):
            ...
            # 2. Cumulant Features (For LSTD Matrices)
            c_phi_val = cumulant_net.apply(cumulant_params, c_obs)
            c_next_phi_val = cumulant_net.apply(cumulant_params, c_next_obs)
        ...
        V_max_dynamic = 1.0 / (1 - config["GAMMA_i"]) # maximum intrinsic values (before scaling by beta)
    ...
    def train(rng):
        ...
        # 3. Initialize Cumulant Network (Learned)
        cumulant_rng, rng = jax.random.split(rng)
        cumulant_net, cumulant_params = networks.initialize_rnd_network(
            cumulant_rng, obs_shape, config["RND_NETWORK_TYPE"], config["NORMALIZE_FEATURES"], config["BIAS"], k_val
        )
        cumulant_target_params = cumulant_params
        cumulant_tx = optax.adam(learning_rate=config.get("LR", 2.5e-4))
        cumulant_train_state = TrainState.create(
            apply_fn=cumulant_net.apply, params=cumulant_params, tx=cumulant_tx
        )
        
        # 4. Initialize Global RND Sigma Accumulator & Buffer
        ...
        def _update_step(runner_state, unused):
            train_state, cumulant_train_state, cumulant_target_params, sigma_state, buffer_state, env_state, last_obs, rng, idx = runner_state
            ...
            def _env_step(env_scan_state, unused):
                ...
                transition = Transition(
                    done, action, value, next_val, jnp.zeros_like(reward), jnp.zeros_like(reward), 
            ...
            w_i, diag_counts = solve_lstd_buffer(
                buffer_state, target_params, rnd_net, cumulant_train_state.params, cumulant_net, Sigma_inv_rnd, k_val, config
            )

            # --- 4. PREPARE CUMULANT TD TARGETS ---
            # target = c_t + gamma * E[c_{t+1} + ...]
            next_cumulant_pred = cumulant_net.apply(cumulant_target_params, traj_batch.next_obs)
            cumulant_targets_batch = phi_rnd + config["GAMMA_i"] * (1 - terminals[..., None]) * next_cumulant_pred

            # --- 5. EVALUATE CURRENT BATCH ---
            batch_next_rho = get_scale_free_bonus(Sigma_inv_rnd, next_phi_rnd)
            
            # Value uses Cumulant Features
            phi_val = cumulant_net.apply(cumulant_train_state.params, traj_batch.obs)
            next_phi_val = cumulant_net.apply(cumulant_train_state.params, traj_batch.next_obs)
            ...
            # UPDATE NETWORKS (PPO + Cumulant)
            def _update_epoch(update_state, unused):
                def _update_minbatch(states, batch_info):
                    train_state, cumulant_train_state = states
                    traj_batch, advantages, targets, cumulant_targs = batch_info
                    
                    # PPO Update
                    grad_fn = jax.value_and_grad(helpers._loss_fn, has_aux=True)
                    (ppo_loss, _), grads = grad_fn(
                        train_state.params, network, traj_batch, advantages, targets, config
                    )
                    train_state = train_state.apply_gradients(grads=grads)
                    
                    # Cumulant Update
                    def _cumulant_loss_fn(params):
                        preds = cumulant_net.apply(params, traj_batch.obs)
                        # MSE over all features and batch elements
                        return jnp.mean(jnp.square(preds - cumulant_targs))
                    
                    cum_loss, cum_grads = jax.value_and_grad(_cumulant_loss_fn)(cumulant_train_state.params)
                    cumulant_train_state = cumulant_train_state.apply_gradients(grads=cum_grads)

                    return (train_state, cumulant_train_state), (ppo_loss, cum_loss)

                train_state, cumulant_train_state, traj_batch, advantages, targets, cumulant_targs, rng = update_state
                rng, _rng = jax.random.split(rng)
                batch = (traj_batch, advantages, targets, cumulant_targs)
                minibatches = helpers.shuffle_and_batch(_rng, batch, config["NUM_MINIBATCHES"])
                
                (train_state, cumulant_train_state), (ppo_loss, cum_loss) = jax.lax.scan(
                    _update_minbatch, (train_state, cumulant_train_state), minibatches
                )
                return (train_state, cumulant_train_state, traj_batch, advantages, targets, cumulant_targs, rng), (ppo_loss, cum_loss)

            initial_update_state = (train_state, cumulant_train_state, traj_batch, advantages, extrinsic_target, cumulant_targets_batch, rng)
            update_state, loss_info = jax.lax.scan(_update_epoch, initial_update_state, None, config["NUM_EPOCHS"])
            train_state, cumulant_train_state, _, _, _, _, rng = update_state
            
            # --- EMA UPDATE FOR CUMULANT TARGET NETWORK ---
            cumulant_target_params = jax.tree_map(
                lambda p, tp: 0.05 * p + 0.95 * tp, 
                cumulant_train_state.params, 
                cumulant_target_params
            )
            ...
            metric.update(
                {
                    ...
                    "cumulant_loss": loss_info[1].mean(),
            ...
            if evaluator is None: 
                ...
                def get_vi(obs):
                    phi_eval_val = cumulant_net.apply(cumulant_train_state.params, obs)
            ...
            runner_state = (train_state, cumulant_train_state, cumulant_target_params, sigma_state, buffer_state, env_state, last_obs, rng, idx + 1)
            return runner_state, metric

        rng, _rng = jax.random.split(rng)
        runner_state = (train_state, cumulant_train_state, cumulant_target_params, initial_sigma_state, initial_buffer_state, env_state, obsv, _rng, 0)
