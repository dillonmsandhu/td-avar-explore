...
SAVE_DIR = "3_26_oracle_beta_decay"
...
def make_train(config):
    ...
    def add_oracle_values_to_metric(
        config, metric, ri_grid, v_e_grid, v_i_grid, traj_batch, evaluator, beta_t
    ):
        """
        ...
        metric.update({
            ...
            "v_i_pred": v_i_grid,
            "v_e_pred": v_e_grid,
            "effective_visits": (beta_t / jnp.maximum(ri_grid, 1e-8))**2,
        })
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            # --- Calculate Cosine Decaying Beta ---
            progress = idx / (0.8 * config["NUM_UPDATES"]) # decays in 80%
            beta_t = config["BONUS_SCALE"] * 0.5 * (1.0 + jnp.cos(jnp.pi * progress))
            ...
            rho_scale = beta_t / sqrt_n  # <-- Now using decayed beta_t
