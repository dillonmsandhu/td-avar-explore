...
def mat_inv_sqrt(A, reg=1e-2):
    ...
    # Ensure eigenvalues are positive to avoid errors (add small epsilon if needed)
...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            sigma_state.update(helpers.sigma_update(sigma_state, traj_batch, phi, alpha))
            ...
            # Intrinsic Critic
            # 1. Whiten phi:
            phi = whiten(sigma_state['S'], reg=1e-2, X=phi, mu=sigma_state['mean_phi'])
            next_phi = whiten(sigma_state['S'], reg=1e-2, X=next_phi, mu=sigma_state['mean_phi'])
