...
class FourRoomsExactValue:
    ...
    def __init__(
        ...
        self.continuing = not episodic
        self.absorbing = absorbing
        self.use_visual_obs = use_visual_obs

        # Assuming `generate_four_rooms_map` is defined elsewhere in your module
        self.env_map = generate_four_rooms_map(self.N)
        self.occupied_map = 1.0 - self.env_map.astype(jnp.float32)
        y, x = jnp.where(self.env_map)
        self.coords = jnp.stack([y, x], axis=1).astype(jnp.int32)
        ...
        # Build specific Transition Matrices
        # Extrinsic dynamics reset to the start unless specifically modeled otherwise
        self.P_ext, self.R_extrinsic = self._build_env_dynamics(
            mode="continuing" if self.continuing else "episodic"
        )
        
        # Intrinsic dynamics loop at the goal if absorbing is True
        if self.absorbing:
            self.P_int, _ = self._build_env_dynamics(mode="absorbing")
        else:
            self.P_int, _ = self._build_env_dynamics(
                mode="continuing" if self.continuing else "episodic"
            )
    ...
    def _build_env_dynamics(self, mode: str) -> Tuple[jax.Array, jax.Array]:
        ...
        for s_idx in range(self.num_states):
            # 1. Goal State Dynamics
            if s_idx == self.goal_idx:
                if mode == "absorbing":
                    next_idx = self.goal_idx
                else: # episodic or continuing loops back to start
    ...
    def compute_true_values(
        ...
        # Extrinsic system uses self.P_ext
        v_e = self.solve_linear_system(pi, self.P_ext, self.R_extrinsic)
        
        # Intrinsic system uses self.P_int
        R_int_sa = jnp.einsum("sam,m->sa", self.P_int, r_int_s)
        v_i = self.solve_linear_system(pi, self.P_int, R_int_sa)
    ...
    def compute_optimal_intrinsic_values(
        ...
        # Extrinsic system uses self.P_ext
        v_e_star = value_iteration(self.P_ext, self.R_extrinsic, self.gamma)
        
        # Intrinsic system uses self.P_int
        R_int_sa = jnp.einsum("sam,m->sa", self.P_int, r_int_s)
        v_i_star = value_iteration(self.P_int, R_int_sa, self.gamma)
...
#             # 1. Goal State Dynamics
#             if s_idx == self.goal_idx:
#                 # If absorbing is True and not continuing, it loops to itself
#                 if self.absorbing and not self.continuing:
#                     next_idx = self.goal_idx
#                 else:
#                     next_idx = self.start_idx if self.continuing else self.goal_idx
