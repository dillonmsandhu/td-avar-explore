...
class SparseMountainCar(MountainCar):
    """JAX implementation of Sparse Reward MountainCar."""

    @property
    def name(self) -> str:
        """Environment name."""
        return "SparseMountainCar-v0"
    ...
    def step_env(
        ...
        """Perform single timestep state transition with sparse reward."""
        # Physics dynamics (identical to original)
        velocity = (
            state.velocity
            + (action - 1) * params.force
            - jnp.cos(3 * state.position) * params.gravity
        ...
        # 1. Goal reached (position and velocity thresholds)
        goal_reached = (position >= params.goal_position) * (
        ...
        done = jnp.logical_or(goal_reached, done_steps)

        # Sparse Reward Calculation:
        # 1.0 if the goal is reached in this step, 0.0 otherwise.
        # Even if the episode times out (done_steps=True), if goal is not reached, reward is 0.
        reward = goal_reached.astype(jnp.float32)

        # Update state struct
        state = EnvState(position=position, velocity=velocity, time=time)
