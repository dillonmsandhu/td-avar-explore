...
SAVE_DIR = 'cov_lstd_whiten_fixed'
...
def sigma_update(sigma_state, transitions, features, α):
    S, mu, t, cov = sigma_state['S'], sigma_state['mean_phi'], sigma_state['t'], sigma_state['cov']
    batch_axes = tuple(range(transitions.done.ndim))
    
    # 1. Update Mean first (or use the previous mean to avoid look-ahead bias)
    current_batch_mean = features.mean(axis=batch_axes)
    new_mu = (1 - α) * mu + α * current_batch_mean
    
    # 2. Center the features using the best available mean
    # centered_features = features - new_mu
    centered_features = features - current_batch_mean
    
    # 3. Update Second Moment (S)
    # vmap the outer product over (N, M) batch dimensions
    S_update = jax.vmap(jax.vmap(lambda x: jnp.outer(x, x)))(features)
    S_b = S_update.mean(axis=batch_axes)
    S_b = 0.5 * (S_b + S_b.T) # Symmetrize
    new_S = (1 - α) * S + α * S_b

    # 3. Update Second Covariance (Cov)
    # vmap the outer product over (N, M) batch dimensions
    cov_update = jax.vmap(jax.vmap(lambda x: jnp.outer(x, x)))(centered_features)
    cov_b = cov_update.mean(axis=batch_axes)
    cov_b = 0.5 * (cov_b + cov_b.T) # Symmetrize
    new_cov = (1 - α) * cov + α * cov_b
    
    return {'S': new_S,'cov': new_cov ,'mean_phi': new_mu, 't': t + 1}

def mat_inv_sqrt(A, reg=1e-2):
    # Eigendecomposition for symmetric/Hermitian matrix
    vals, vecs = jnp.linalg.eigh(A + reg * jnp.eye(A.shape[0]))
    # Inverse square root of eigenvalues
    inv_sqrt_vals = 1.0 / jnp.sqrt(vals)
    # Reconstruct matrix
    return vecs @ jnp.diag(inv_sqrt_vals) @ vecs.T

def whiten(A, reg, X, mu):
    """A is (k,k), x is  (..., k)"""
    A_inv_sqrt = mat_inv_sqrt(A, reg) # k x k
    centered = X - mu[None, None, ...]
    x_whitened = jnp.einsum('kj,...j->...k', A_inv_sqrt, centered)
    return x_whitened
...
def make_train(config):
    ...
    REG_INIT = config.get("A_REG_INIT", 1e-2)
    ...
    def train(rng):
        ...
        initial_sigma_state = {
            'cov': jnp.eye(k) * config['GRAM_REG'],
            'S': jnp.eye(k) * config['GRAM_REG'],
            'N': 0, 't': 1,
            'mean_phi': jnp.zeros(k)
        }
        ...
        def _update_step(runner_state, unused):
            ...
            alpha = alpha_fn(sigma_state['t'])
            sigma_state.update(sigma_update(sigma_state, traj_batch, phi, alpha))
            # sigma_state['mean_phi'] = EMA(alpha, phi.mean(0).mean(0), sigma_state['mean_phi'])
            ...
            # Intrinsic Critic
            # 1. Whiten phi:
            # phi = whiten(sigma_state['cov'], reg=config['WHITEN_REG'], X=phi, mu=sigma_state['mean_phi'])
            # next_phi = whiten(sigma_state['cov'], reg=config['WHITEN_REG'], X=next_phi, mu=sigma_state['mean_phi'])
            # phi = whiten(sigma_state['cov'], reg=config['WHITEN_REG'], X=phi, mu=phi.mean(0).mean(0))
            # next_phi = whiten(sigma_state['cov'], reg=config['WHITEN_REG'], X=next_phi, mu=next_phi.mean(0).mean(0))
            ...
            metric.update({
                ...
                "w_norm": jnp.linalg.norm(lstd_state['w']),
                "w_unreg_norm": jnp.linalg.norm(lstd_state['w_unreg']),
            })
