...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            # Intrinsic Critic
            # 1. Whiten phi:
            phi = whiten(sigma_state['S'], reg=config['WHITEN_REG'], X=phi, mu=sigma_state['mean_phi'])
            next_phi = whiten(sigma_state['S'], reg=config['WHITEN_REG'], X=next_phi, mu=sigma_state['mean_phi'])
